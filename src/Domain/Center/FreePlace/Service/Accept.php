...
final class Accept extends Admin{
     ...
     */
    public function accept(int $id, array $post) {
        ...
        if(!empty($signed_result)) {
            ...
            if($this->logCreateRepository->insert($log) == 0) {
                throw new DomainException("Error to accept free place");
            }
            $this->updateRepository->updateById($id, array("status_id" => $this->status_id));
        } else {
